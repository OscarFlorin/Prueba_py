#Este es mi primera prueba para Python en github.

print("Hola mundo")

#Este es mi otra prueba
numero_1= int(input("Ingresa el primer numero positivo: "))
numero_2= int(input("Ingresa el segundo numero positivo: "))


suma = numero_1+numero_2
print(suma)

""" 
Estos son los comandos para poder sincronizar archivos a GitHub
$ git add . #Este comando prepara todas las modificaciones 
$ git commit -m "Ejemplo de mensaje para commit" #Este comando captura una snapshot de los cambios preparados en este momento
$ git push #Este comando sincroniza el archivo a github """

print(f"Multiplicar numero 1 {numero_1} * numero 2 {numero_1}= {numero_2*numero_2}")



